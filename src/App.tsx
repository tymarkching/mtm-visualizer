...
export default function App() {
  ...
  const [customTextFontFamily, setCustomTextFontFamily] = useState<string>('Inter');
  ...
  const [monitorIsMuted, setMonitorIsMuted] = useState<boolean>(false);
  ...
  const monitorVolumeRef = useRef<number>(monitorVolume);
  ...
  useEffect(() => {
    monitorIsMutedRef.current = monitorIsMuted;
  }, [monitorIsMuted]);
  ...
  const initAudioSystem = () => {
    ...
    try {
      ...
      // Create primary trackGainNode representing Background Music Routing Mix track volume and mute
      const trackGainNode = ctx.createGain();
      trackGainNode.gain.value = isMuted ? 0 : volume;

      // Connect Panner Output -> trackGainNode (applies true audio track volume before analyzer and captures)
      pannerOutput.connect(trackGainNode);

      // trackGainNode -> core Analyser
      trackGainNode.connect(analyser);

      // Master Gain for Volume mapping (Fidelity master always at 1.0, with fade effects applied to it)
      const gainNode = ctx.createGain();
      gainNode.gain.value = 1.0;

      // Local Preview / Monitor Gain Node connected only to physical hardware speakers
      const monitorGainNode = ctx.createGain();
      monitorGainNode.gain.value = monitorIsMuted ? 0 : monitorVolume;

      // Connect AnalyserNode to Master Gain Node
      analyser.connect(gainNode);

      // Route main gain node output to:
      // A) speakers (via monitorGainNode for fully isolated dynamic volume/mute slider dampening of hardware preview speakers)
      ...
      previewGainNodeRef.current = monitorGainNode;
  ...
  // Keep track volume & mute aligned with trackGainNode
  useEffect(() => {
    if (trackGainNodeRef.current && audioContextRef.current) {
  ...
  // Keep monitor volume & mute aligned with monitorGainNode
  useEffect(() => {
    if (monitorGainNodeRef.current && audioContextRef.current) {
      monitorGainNodeRef.current.gain.setValueAtTime(
        monitorIsMuted ? 0 : monitorVolume,
        audioContextRef.current.currentTime
      );
    }
  }, [monitorVolume, monitorIsMuted]);
  ...
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      if (overlayVideoRef.current && visualsRef.current.overlayVideoUrl) {
        ...
        try {
          ...
          if (overlayVid.readyState >= 2) {
            ...
            // Set dynamic composition blend mode
            const blendMode = visualsRef.current.overlayBlendMode || 'normal';
            if (blendMode === 'screen') {
            ...
            let opacity = visualsRef.current.overlayOpacity !== undefined ? visualsRef.current.overlayOpacity / 100 : 1.0;
            
            // Beat reaction pulse dynamic calculation
            let pulseScaleFactor = 1.0;
            if (visualsRef.current.overlayBeatPulse && visualsRef.current.overlayPulseIntensity) {
              const pulseIntensityFactor = visualsRef.current.overlayPulseIntensity / 100;
              const pulseScaleAmt = pulseIntensityFactor * bassReactiveFactor * 0.4;
              pulseScaleFactor = 1.0 + pulseScaleAmt;
              
              // Pulsate opacity on beats
              opacity *= (1.0 - pulseIntensityFactor * 0.3 * (1.0 - bassReactiveFactor));
            }
        ...
        overlayImagesRef.current.forEach((imgItem) => {
          ...
          if (imgEl && (imgEl.complete || imgEl.readyState >= 2)) {
            ...
            try {
              ...
              let targetWidth = canvas.width * 0.5 * (imgItem.scale / 100);
              let targetHeight = targetWidth / (imgItem.aspectRatio || 1.0);
              ...
              // Sticker float & rotation animations
              let floatY = 0;
              let rotation = 0;
              if (visualsRef.current.stickerFloatRotate && visualsRef.current.stickerFloatSpeed) {
                const animSpeed = visualsRef.current.stickerFloatSpeed;
                const time = Date.now() * (animSpeed / 100) * 0.004;
                floatY = Math.sin(time + overlayImagesRef.current.indexOf(imgItem) * 1.5) * 15;
                rotation = Math.sin(time * 0.5 + overlayImagesRef.current.indexOf(imgItem) * 1.5) * 0.15;
              }

              const centerXVal = canvas.width * (imgItem.x / 100);
              const centerYVal = canvas.height * (imgItem.y / 100) + floatY;

              ctx.translate(centerXVal, centerYVal);
              if (rotation !== 0) {
                ctx.rotate(rotation);
              }

              // Draw image centered at (0,0)
              ctx.drawImage(imgEl, -targetWidth / 2, -targetHeight / 2, targetWidth, targetHeight);
              ...
              if (activeTabRef.current === 'overlay' && !isExportingRef.current && selectedOverlayImageIdRef.current === imgItem.id) {
                ...
                ctx.shadowColor = 'rgba(0, 0, 0, 0.9)';
                ctx.shadowBlur = 5;
                ctx.fillText('Drag corners to scale sticker / Center to move', 0, -targetHeight / 2 - 8);
      ...
      if (hasText || hasStickers) {
        ...
        let marqueeX = finalX;
        let isMarqueeActive = !!visualsRef.current?.customTextMarquee;
        ...
        if (isMarqueeActive) {
          ...
          const textWidth = ctx.measureText(customTextRef.current || '').width;
          ctx.restore();

          const speed = visualsRef.current.customTextScrollSpeed !== undefined ? visualsRef.current.customTextScrollSpeed : 2;
          const totalWidth = canvas.width + textWidth;
          const progress = (Date.now() * speed * 0.05) % totalWidth;
          const startX = canvas.width + textWidth / 2;
          marqueeX = startX - progress;
        }
        ...
        if (hasStickers) {
          ...
          const emojiSize = visualsRef.current.stickerSize !== undefined ? visualsRef.current.stickerSize : Math.max(24, customTextFontSizeRef.current * 0.9);
          ...
          const stickerXVal = visualsRef.current.stickerX !== undefined ? (visualsRef.current.stickerX / 100) * canvas.width : marqueeX;
          const stickerYVal = visualsRef.current.stickerY !== undefined ? (visualsRef.current.stickerY / 100) * canvas.height : (finalY + (hasText ? (customTextFontSizeRef.current * 0.75 + 15) : 0));

          const totalEmojis = activeStickersRef.current.length;
          const spacing = emojiSize * 1.35;
          const startX = stickerXVal - ((totalEmojis - 1) * spacing) / 2;
          ...
          activeStickersRef.current.forEach((emoji, index) => {
            ...
            ctx.fillText(emoji, currentX, stickerYVal + floatOffset);
          });
      ...
      // 8. Draw Built-in Brand Watermark if enabled in visuals (e.g. at selected alignment or corner)
      if (visualsRef.current?.renderWatermark) {
        ctx.save();
        const textOpacity = visualsRef.current?.textWatermarkOpacity !== undefined ? visualsRef.current.textWatermarkOpacity : 0.35;
        ctx.fillStyle = `rgba(255, 255, 255, ${textOpacity})`; // subtle, semi-transparent modern overlay font color with decoupled opacity
        const fontSz = visualsRef.current?.textWatermarkFontSize || visualsRef.current?.watermarkFontSize || 12;
        ctx.font = `500 ${fontSz}px "Inter", sans-serif`;
        ctx.shadowColor = 'rgba(0, 0, 0, 0.4)';
        ctx.shadowBlur = 4;

        const align = visualsRef.current?.textWatermarkAlignment || 'bottom-right';
        let tx = canvas.width - 24;
        let ty = canvas.height - 24;
        let tAlign: CanvasTextAlign = 'right';
        let tBaseline: CanvasTextBaseline = 'bottom';
        ...
        if (align === 'top-left') {
          ...
          tAlign = 'right';
          tBaseline = 'bottom';
        } else if (align === 'manual') {
          const xPct = visualsRef.current?.textWatermarkX !== undefined ? visualsRef.current.textWatermarkX : 90;
          const yPct = visualsRef.current?.textWatermarkY !== undefined ? visualsRef.current.textWatermarkY : 90;
          tx = (xPct / 100) * canvas.width;
          ty = (yPct / 100) * canvas.height;
          tAlign = 'center';
          tBaseline = 'middle';
        } else {
          // Check legacy position parameter if present
          const pos = visualsRef.current?.watermarkPosition || 'top-right';
          if (pos === 'top-left') {
            tx = 24;
            ty = 24;
            tAlign = 'left';
          ...
          <div className="h-[calc(100vh-110px)] overflow-y-auto pr-1 space-y-4 custom-scrollbar p-5 md:p-6 bg-zinc-950">
            ...
            {activeTab === 'track' && (
              ...
              >
                    ...
                    <div className="flex items-center justify-between px-1 mb-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <button 
                          ...
                          className="text-zinc-500 hover:text-brand-green-hover p-1 rounded transition-colors text-[10px] font-mono flex items-center space-x-1" 
                          ...
                          return (
                            <motion.div
                              ...
                              className={`flex items-center justify-between rounded-lg p-3.5 cursor-pointer transition-all border ${
                                isActive
                                  ? 'bg-lime-950/40 border-brand-green/50 text-brand-green-hover'
                            ...
                            >
                              <div className="flex items-center space-x-3 overflow-hidden">
                                <div className={`w-9 h-9 rounded flex items-center justify-center shrink-0 ${isActive ? 'bg-brand-green/10 border border-brand-green/20 text-brand-green-hover' : 'bg-zinc-800/50 text-zinc-500'}`}>
                                ...
                                <div className="overflow-hidden text-left">
                                  ...
                                  {isActive ? (
                                    <p className="text-[10px] text-brand-green/80 font-mono truncate mt-0.5 uppercase">Now Playing</p>
                                  ) : (
                              ...
                              <button
                                ...
                                className={`${isActive ? 'text-brand-green/50 hover:text-red-400' : 'text-zinc-600 hover:text-red-400'} p-1.5 rounded hover:bg-red-500/15 transition-all shrink-0`}
                                title="Remove Track"
                              >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider font-mono flex items-center space-x-2">
                      <Type className="w-3.5 h-3.5 text-brand-green" />
                    ...
                    <button
                      ...
                      className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                        titleOverlay.visible ? 'bg-brand-green' : 'bg-zinc-800'
                      }`}
                    >
                    ...
                    <div className="p-3.5 bg-zinc-900 border border-zinc-800 rounded-lg text-xs space-y-3.5">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white focus:outline-none focus:border-brand-green uppercase font-semibold text-xs"
                        />
                      </div>
                ...
                <div className="space-y-4">
                          ...
                          <div key={item.id} className="bg-zinc-950/25 rounded-lg border border-zinc-900/50 p-1 space-y-1">
                            <button
                              ...
                              className={`w-full py-1.5 px-2.5 rounded-md border text-left flex items-center justify-between transition-all text-[11px] leading-tight cursor-pointer ${
                                active
                                  ? 'bg-brand-green/15 border-brand-green/60 text-white font-medium'
                            ...
                            >
                              ...
                              <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center transition-all ${
                                active
                                  ? 'border-brand-green-hover bg-brand-green/20 text-lime-300'
                              ...
                              <div className="px-2.5 py-2 bg-black/40 border border-zinc-900/40 rounded-md space-y-2 mt-1 mx-0.5">
                                <div className="space-y-1">
                                  <div className="flex justify-between items-center text-[9px] font-mono">
                                    ...
                                    <span className="text-brand-green-hover font-semibold font-mono">{styleX}%</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="0"
                                ...
                                <div className="space-y-1">
                                  <div className="flex justify-between items-center text-[9px] font-mono">
                                    ...
                                    <span className="text-brand-green-hover font-semibold font-mono">{styleY}%</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="0"
                                ...
                                <div className="space-y-1">
                                  <div className="flex justify-between items-center text-[9px] font-mono">
                                    <span className="text-zinc-500 uppercase font-bold text-brand-green-hover">Vertical Scale / Line Height</span>
                                    <span className="text-brand-green-hover font-semibold font-mono">{styleScale.toFixed(2)}x</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="0.1"
                                  ...
                                  />
                                   ...
                                   <div className="flex justify-between items-center text-[9px] font-mono">
                                     <span className="text-zinc-500 uppercase font-bold text-brand-green-hover">Horizontal Scale / Thickness</span>
                                     <span className="text-brand-green-hover font-semibold font-mono">{styleHorizontalScale}px</span>
                                   </div>
                                   <input
                                     type="range"
                                     min="1"
                                 ...
                                 <div className="space-y-1 pt-1.5 border-t border-zinc-900/40">
                                   <div className="flex justify-between items-center text-[9px] font-mono">
                                     ...
                                     <span className="text-brand-green-hover font-semibold font-mono">{styleMasterScale}%</span>
                                   </div>
                                   <input
                                     type="range"
                                     min="10"
                                 ...
                                 <div className="space-y-1">
                                   <div className="flex justify-between items-center text-[9px] font-mono">
                                     ...
                                     <span className="text-brand-green-hover font-semibold font-mono">{styleHorizontalSpan}%</span>
                                   </div>
                                   <input
                                     type="range"
                                     min="20"
                                 ...
                                 <div className="space-y-1 pt-1.5 border-t border-zinc-900/40">
                                   <div className="flex justify-between items-center text-[9px] font-mono">
                                     ...
                                     <span className="text-brand-green-hover font-semibold font-mono">{styleSpringTension.toFixed(2)}</span>
                                   </div>
                                   <input
                                     type="range"
                                     min="0.01"
                                 ...
                                 <div className="space-y-1">
                                   <div className="flex justify-between items-center text-[9px] font-mono">
                                     ...
                                     <span className="text-brand-green-hover font-semibold font-mono">{styleSpringDampening.toFixed(2)}</span>
                                   </div>
                                   <input
                                     type="range"
                                     min="0.05"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full bg-[#0a0a0f] border border-gray-950 rounded-lg p-2 text-zinc-300 text-xs focus:outline-none focus:border-brand-green font-medium"
                    >
                  ...
                  <div className="space-y-3 bg-[#07070a]/50 p-2.5 rounded-lg border border-zinc-950/60">
                    <div className="space-y-1 text-left">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[11px] font-semibold text-brand-green-hover font-mono">
                        ...
                        <button
                          ...
                          className={`py-1.5 px-1 rounded font-mono text-[9px] font-semibold transition-all ${
                            (visuals.colorMode || 'gradient') === mode.id
                              ? 'bg-brand-green border border-brand-green text-zinc-950 font-bold'
                      ...
                      <div className="space-y-1 bg-zinc-950/40 p-2 rounded border border-zinc-900 mt-2">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.useCustomGradientDirection ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                        ...
                        {visuals.useCustomGradientDirection && (
                          <div className="pt-2 space-y-1">
                            ...
                            <input
                              ...
                              className="w-full accent-brand-green h-1.5 rounded-full cursor-pointer bg-zinc-900 appearance-none"
                            />
                          </div>
                        )}
                      </div>
                    ...
                    <div className="space-y-1 bg-zinc-950/40 p-2 rounded border border-zinc-900 mt-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.useCustomWaveformColor ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-zinc-900/50">
                        <div>
                          <span className="font-semibold text-zinc-300 block font-sans text-[11px]">Frequency-Based Bar Coloring</span>
                          <span className="text-[10px] text-zinc-500 block font-sans mt-0.5">Colors individual bars dynamically based on their frequency height instead of a static gradient</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => setVisuals(prev => ({ ...prev, frequencyBasedColoring: !prev.frequencyBasedColoring }))}
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.frequencyBasedColoring ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                            visuals.frequencyBasedColoring ? 'translate-x-3.5' : 'translate-x-0'
                          }`} />
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3.5 bg-zinc-900 border border-zinc-800 p-4 rounded-lg text-xs">
                    ...
                    <div className="space-y-1 pb-2 mb-2 border-b border-zinc-800">
                      ...
                      <select
                        className="w-full bg-zinc-950 border border-zinc-800 rounded p-1.5 text-zinc-300 font-sans text-[11px]"
                        onChange={(e) => {
                          const preset = e.target.value;
                          if (preset === 'chill') {
                            setVisuals(prev => ({ ...prev, sensitivity: 0.8, barFrequencyCount: 32 }));
                          } else if (preset === 'balanced') {
                            setVisuals(prev => ({ ...prev, sensitivity: 1.2, barFrequencyCount: 64 }));
                          } else if (preset === 'high-energy') {
                            setVisuals(prev => ({ ...prev, sensitivity: 1.8, barFrequencyCount: 128 }));
                          } else if (preset === 'extreme') {
                            setVisuals(prev => ({ ...prev, sensitivity: 2.5, barFrequencyCount: 256 }));
                          }
                        }}
                      >
                        <option value="">Select Preset...</option>
                        <option value="chill">Chill (Low Reactiveness, Minimal Bars)</option>
                        <option value="balanced">Balanced (Standard Dynamics)</option>
                        <option value="high-energy">High Energy (Fast & Dense)</option>
                        <option value="extreme">Extreme (Maximum Sensitivity & Density)</option>
                      </select>
                    </div>
                    ...
                    <div className="space-y-2 border-t border-zinc-850 pt-2 bg-transparent">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.beatLock ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center space-x-3 p-2 bg-[#0c0c14]/50 border border-zinc-950/60 rounded">
                        <div className="relative flex items-center justify-center w-5 h-5 flex-shrink-0">
                          ...
                          {/* Inner pulsing core */}
                          <div 
                            className="absolute w-2.5 h-2.5 rounded-full bg-brand-green shadow-[0_0_10px_#3b82f6]"
                            style={{
                          ...
                          <div 
                            className="absolute w-5 h-5 rounded-full border border-brand-green/80"
                            style={{
                        ...
                        <div className="flex-1 text-left">
                          ...
                          <span className="text-[10px] text-zinc-500 block font-sans">
                            Pulse matched to <strong className="text-brand-green-hover font-mono font-semibold">{(visuals.beatLockBpm || 120)} BPM</strong>
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.flashOnBeat ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    {visuals.flashOnBeat && (
                      <div className="pl-3 py-2 px-2.5 mt-0.5 bg-[#050508]/60 border-l-2 border-brand-green/80 space-y-2.5 rounded-r border-y border-r border-zinc-950/40">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full accent-brand-green h-1 rounded-full cursor-pointer bg-zinc-950 appearance-none"
                          />
                        </div>
                              ...
                              <button
                                ...
                                className={`px-1.5 py-1 text-[9px] font-medium font-sans border rounded transition-all text-center cursor-pointer ${
                                  (visuals.flashColorMode || 'white') === opt.id
                                    ? 'bg-brand-green/20 text-brand-green-hover border-brand-green/50'
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.flipWaveform ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                        ...
                        <div className="space-y-3 pt-1 border-t border-zinc-950/40 animate-fade-in">
                                ...
                                return (
                                  <button
                                    ...
                                    className={`py-2 px-1 rounded-md font-sans transition-all cursor-pointer flex flex-col items-center justify-center gap-1.5 ${
                                      isActive
                                        ? 'bg-brand-green/95 border border-brand-green text-white shadow-md shadow-lime-950/40'
                          ...
                          <div className="flex items-center justify-between pt-1">
                            ...
                            <button
                              ...
                              className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                                visuals.symmetryColorInversion ? 'bg-brand-green' : 'bg-zinc-800'
                              }`}
                            >
                              <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.colorInvertOnBeat ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.colorBurstOnBeat ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.beatReactiveColorShift ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      {visuals.beatReactiveColorShift && (
                        <div className="pt-2 border-t border-zinc-950/40 space-y-1.5 animate-fade-in">
                          <div className="flex justify-between items-center text-[9px] font-mono">
                            ...
                            <span className="text-brand-green-hover font-bold">{(visuals.colorShiftIntensity !== undefined ? visuals.colorShiftIntensity : 5.0).toFixed(1)}x</span>
                          </div>
                          <input
                            type="range"
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.cycleColors ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="grid grid-cols-1 gap-3 pt-1">
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          className="w-full bg-zinc-950 border border-zinc-850 rounded p-1.5 text-zinc-300 text-xs focus:outline-none focus:border-brand-green font-medium"
                        >
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          <div className="flex items-center justify-between">
                            ...
                            <select
                              ...
                              className="flex-1 bg-zinc-950 border border-zinc-850 rounded p-1 text-zinc-300 text-[10px] focus:outline-none focus:border-brand-green"
                            >
                    ...
                    <div className="border-t border-zinc-800/60 pt-3.5 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.spectrumAnalyzer ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div id="audio-reactive-fx-section" className="border-t border-zinc-800/60 pt-4 mt-4 space-y-3">
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60 mt-1">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.reactiveTextGlow ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="border-t border-zinc-800/60 pt-4 mt-4 space-y-3 font-sans">
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.kaleidoscope ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.chromaticAberration ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.earthquakeCameraShake ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div id="video-asset-overlays-section" className="border-t border-zinc-800/60 pt-4 mt-4 space-y-3 font-sans">
                        ...
                        <div className="flex items-center justify-between p-2.5 bg-[#07070a]/80 rounded border border-zinc-950/60">
                          ...
                          <button
                            ...
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.showProgressBar ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                          ...
                          <div className="space-y-3.5 p-3.5 bg-[#07070a]/60 rounded border border-zinc-900 mt-2 text-left animate-in fade-in duration-200">
                            ...
                            <div className="grid grid-cols-2 gap-2.5">
                              <div>
                                ...
                                <select
                                  ...
                                  className="w-full bg-zinc-950 border border-zinc-850 rounded px-2 py-1.5 text-[11px] text-zinc-300 focus:outline-none focus:border-brand-green transition-colors font-sans"
                                >
                            ...
                            <div className="flex items-center justify-between border-t border-zinc-900/60 pt-2.5 bg-transparent">
                              ...
                              <button
                                ...
                                className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                                  visuals.progressBarRoundCaps !== false ? 'bg-brand-green' : 'bg-zinc-800'
                                }`}
                              >
                                <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
            ...
            {activeTab === 'particles' && (
              ...
              >
                ...
                <div className="space-y-4">
                  {/* Master Switch UI */}
                  <div className="flex items-center justify-between p-3.5 rounded-xl bg-brand-green/5 border border-brand-green/15 shadow-sm shadow-brand-green/5">
                    <div>
                      <span className="font-bold text-brand-green-hover block font-sans text-xs uppercase tracking-wide">Enable Particle Engine</span>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-zinc-950 border border-zinc-850 rounded px-2.5 py-1.5 text-zinc-300 font-sans text-[11px] cursor-pointer focus:outline-none focus:ring-1 focus:ring-brand-green-custom"
                    >
                  ...
                  <div className="space-y-4 bg-zinc-900 border border-zinc-800 p-4 rounded-lg text-xs">
                    ...
                    <div className="flex items-center justify-between p-2 bg-[#07070a]/40 rounded border border-zinc-950/40">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.audioReactiveGravity ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850 mt-2">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.straightMotionOverride ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850 mt-2">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.chaoticWindDrift ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.beatReactive ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.isAngularBurstActive ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.beatBurst ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.enableApexAttractor ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.enablePhysics ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="flex items-center justify-between p-2.5 bg-zinc-950/60 rounded border border-zinc-850 mt-2">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          particlesSet.enableParticleCollisions ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="mt-2.5 pt-3 border-t border-zinc-900 space-y-3">
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.colorInvertOnBeat ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.colorBurstOnBeat ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.beatReactiveColorShift ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.cycleColors ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="pt-4 border-t border-zinc-900 space-y-2">
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.particleTwinkle ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.orbitalSway ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      <div className="flex items-center justify-between p-2 pb-2.5 bg-zinc-950/40 rounded-lg border border-zinc-900">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            particlesSet.particleGlitch ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    <div className="pt-4 border-t border-zinc-900 space-y-3.5 text-left">
                      <div className="flex items-center justify-between pb-1">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            (visuals.enableFireworks ?? false) ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                        ...
                        <div className="space-y-3 bg-[#0a0a0f]/40 p-2.5 rounded-lg border border-zinc-900/60 transition-all">
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between items-center text-[10px] font-mono">
                              ...
                              <span className="text-brand-green-hover font-semibold">{(visuals.fireworksAltitude ?? 50)}%</span>
                            </div>
                            <input
                              type="range"
                              min="10"
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between items-center text-[10px] font-mono">
                              ...
                              <span className="text-brand-green-hover font-semibold">{(visuals.fireworksRadius ?? 1.0).toFixed(1)}x</span>
                            </div>
                            <input
                              type="range"
                              min="0.2"
                          ...
                          <div className="space-y-1">
                            <div className="flex justify-between items-center text-[10px] font-mono">
                              ...
                              <span className="text-brand-green-hover font-semibold">{(visuals.fireworksSparkSize ?? 1.5).toFixed(1)}px</span>
                            </div>
                            <input
                              type="range"
                              min="0.5"
                              ...
                              className="w-full h-1 bg-[#050508] rounded appearance-none cursor-pointer accent-brand-green"
                            />
                          </div>
                          ...
                          <div className="space-y-1 pt-1">
                            ...
                            <select
                              ...
                              className="w-full bg-[#050508] border border-zinc-900 rounded text-[11px] px-2 py-1.5 font-mono text-zinc-300 focus:outline-none focus:ring-1 focus:ring-brand-green cursor-pointer"
                            >
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-zinc-950">
                            ...
                            <input
                              ...
                              className="w-4 h-4 accent-brand-green cursor-pointer rounded border-zinc-900 bg-[#050508]"
                            />
                          </div>
                        </div>
                      )}
                        ...
                        <div className={`p-3 rounded-lg flex items-center justify-between text-xs border ${
                          ...
                          <div className="flex items-center space-x-2.5 overflow-hidden">
                            <div className="w-8 h-8 rounded bg-zinc-950 flex items-center justify-center shrink-0 border border-zinc-850">
                              <FileVideo className={`w-4 h-4 ${background.type === 'video' ? 'text-brand-green animate-pulse' : 'text-zinc-500'}`} />
                            </div>
                            <div className="truncate flex flex-col">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer ${
                          background.enableBeatReaction ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                    ...
                    {background.enableBeatReaction && (
                      <div className="space-y-1.5 pt-2 border-t border-zinc-800/50">
                        ...
                        <select
                          ...
                          className="w-full bg-[#0a0a0f] border border-zinc-850 rounded-lg p-2 text-zinc-300 text-xs focus:outline-none focus:border-brand-green font-medium cursor-pointer"
                        >
                ...
                <div className="space-y-4">
                    ...
                    {!visuals.overlayVideoUrl ? (
                      ...
                      <div className="p-3 bg-zinc-950 border border-zinc-850 rounded-lg flex items-center justify-between">
                        <div className="flex items-center space-x-2.5 truncate">
                          <div className="w-8 h-8 rounded bg-lime-950 flex items-center justify-center border border-lime-900">
                            <FileVideo className="w-4 h-4 text-brand-green" />
                          </div>
                  ...
                  {visuals.overlayVideoUrl && (
                    <div className="bg-zinc-900 border border-zinc-855 p-4 rounded-lg space-y-4">
                      ...
                      {/* DYNAMIC CSS BLEND MODES */}
                      <div className="space-y-1.5 pt-3.5 border-t border-zinc-850/50">
                        <label className="block text-[10px] text-zinc-400 font-mono uppercase font-semibold text-left">Overlay Blend Mode</label>
                        <select
                          value={visuals.overlayBlendMode || 'normal'}
                          onChange={(e) => setVisuals(prev => ({ ...prev, overlayBlendMode: e.target.value as any }))}
                          className="w-full bg-[#0a0a0f] border border-zinc-800 rounded-lg p-2 text-zinc-350 text-xs focus:outline-none focus:border-brand-green font-medium cursor-pointer"
                        >
                        ...
                        <p className="text-[9.5px] text-zinc-500 leading-normal font-sans text-left">
                          Applies the selected CSS 'mix-blend-mode' composition to the foreground overlay video canvas drawing context.
                        </p>
                      </div>

                      {/* BEAT-REACTIVE VIDEO PULSING */}
                      <div className="space-y-3 pt-3.5 border-t border-zinc-850/50">
                        <div className="flex items-center justify-between">
                          <div>
                            <span className="font-semibold text-zinc-350 block font-sans text-[10px] uppercase text-left">Beat-Reactive Video Pulsing</span>
                            <span className="text-[9px] text-zinc-500 block font-sans mt-0.5 text-left">Scale overlay size and opacity reactively on bass peak drops</span>
                          </div>
                          <button
                            type="button"
                            onClick={() => setVisuals(prev => ({ ...prev, overlayBeatPulse: !prev.overlayBeatPulse }))}
                            className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                              visuals.overlayBeatPulse ? 'bg-brand-green' : 'bg-zinc-800'
                            }`}
                          >
                            <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                              visuals.overlayBeatPulse ? 'translate-x-3.5' : 'translate-x-0'
                            }`} />
                          </button>
                        </div>

                        {visuals.overlayBeatPulse && (
                          <div className="space-y-1.5 text-left border-l-2 border-brand-green/30 pl-2">
                            <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                              <span>Pulse Intensity</span>
                              <span className="text-white font-semibold">
                                {visuals.overlayPulseIntensity !== undefined ? visuals.overlayPulseIntensity : 0} %
                              </span>
                            </div>
                            <input
                              type="range"
                              ...
                              max="100"
                              step="5"
                              value={visuals.overlayPulseIntensity !== undefined ? visuals.overlayPulseIntensity : 0}
                              onChange={(e) => setVisuals(prev => ({ ...prev, overlayPulseIntensity: parseInt(e.target.value) }))}
                              className="w-full accent-brand-green cursor-pointer"
                            />
                          </div>
                        )}
                  ...
                  <div className="space-y-4">
                    {/* Sticker Physics Animations card */}
                    <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-lg space-y-3.5">
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-semibold text-zinc-350 block font-sans text-[10px] uppercase text-left">Sticker Float & Rotation</span>
                          <span className="text-[9px] text-zinc-500 block font-sans mt-0.5 text-left">Applies smooth floating sine-waves and continuous rotations to sticker overlays</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => setVisuals(prev => ({ ...prev, stickerFloatRotate: !prev.stickerFloatRotate }))}
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.stickerFloatRotate ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                            visuals.stickerFloatRotate ? 'translate-x-3.5' : 'translate-x-0'
                          }`} />
                        </button>
                      </div>

                      {visuals.stickerFloatRotate && (
                        <div className="space-y-1.5 text-left border-l-2 border-brand-green/30 pl-2">
                          <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                            <span>Animation Speed</span>
                            <span className="text-white font-semibold">
                              {visuals.stickerFloatSpeed !== undefined ? visuals.stickerFloatSpeed : 0} %
                            </span>
                          </div>
                          <input
                            type="range"
                            ...
                            max="100"
                            step="5"
                            value={visuals.stickerFloatSpeed !== undefined ? visuals.stickerFloatSpeed : 0}
                            onChange={(e) => setVisuals(prev => ({ ...prev, stickerFloatSpeed: parseInt(e.target.value) }))}
                            className="w-full accent-brand-green cursor-pointer"
                          />
                        </div>
                      )}
                    ...
                    <div className="bg-zinc-900 border border-zinc-850 p-4 rounded-lg space-y-3">
                      ...
                      <div className="relative border-2 border-dashed border-zinc-800 rounded-lg p-5 flex flex-col items-center justify-center text-center hover:border-zinc-700 transition-all cursor-pointer">
                        <input
                          ...
                          className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                        />
                        <Upload className="w-5 h-5 text-brand-green mb-2" />
                                    ...
                                    <div className="space-y-2 bg-[#0c0c14]/40 p-2.5 rounded border border-zinc-800/40">
                                      <div className="flex items-center justify-between">
                                        ...
                                        <button
                                          ...
                                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer ${
                                            img.enableBeatReaction ? 'bg-brand-green' : 'bg-zinc-800'
                                          }`}
                                        >
                                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
              ...
              >
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-3.5">
                    <div className="space-y-1.5 text-left">
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full bg-[#050508] border border-zinc-800 focus:border-brand-green/50 rounded p-2 text-xs text-white placeholder-zinc-700 outline-none transition-all font-sans resize-none"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3.5 pt-1.5 font-sans">
                      ...
                      <div className="space-y-1.5 text-left">
                        ...
                        <select
                          ...
                          onChange={(e) => setCustomTextFontFamily(e.target.value)}
                          className="w-full bg-[#050508] border border-zinc-800 rounded p-2 text-xs text-white outline-none cursor-pointer transition-all"
                        >
                          ...
                          <optgroup label="Serif / Cinematic">
                          ...
                          <optgroup label="Script / Decorative">
                            ...
                            <option value="Pacifico">Pacifico (Retro Wave)</option>
                            <option value="Great Vibes">Great Vibes</option>
                            <option value="Satisfy">Satisfy (Vibe)</option>
                          </optgroup>
                          <optgroup label="Legacy / Tech">
                        ...
                        {!visuals.textGradient ? (
                          <div className="flex items-center space-x-2">
                            ...
                            <input
                              ...
                              onChange={(e) => setCustomTextColor(e.target.value)}
                              maxLength={7}
                              className="w-full bg-[#050508] border border-zinc-800 focus:border-brand-green/50 rounded p-2 text-xs text-white placeholder-zinc-700 outline-none transition-all font-mono"
                            />
                          </div>
                        ) : (
                          <div className="space-y-2">
                                  ...
                                  <button
                                    ...
                                    className={`flex-1 py-1 px-0 text-[9px] rounded flex items-center justify-center transition-all ${
                                      (visuals.textGradientAngle || 'vertical') === opt.value
                                        ? 'bg-brand-green/20 text-brand-green-hover font-semibold'
                                        : 'text-zinc-500 hover:text-zinc-300'
                                    }`}
                                  >
                    ...
                    <div className="space-y-1.5 text-left pt-1.5">
                      <div className="flex justify-between items-center text-[10px] font-mono">
                        ...
                        <span className="text-brand-green-hover font-semibold">{customTextFontSize}px</span>
                      </div>
                      <input
                        type="range"
                        min="12"
                    ...
                    {/* Premium Text Render Effects Dropdown */}
                    <div className="space-y-1.5 text-left pt-1.5 border-t border-zinc-850/40">
                      <label className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-wide">Premium Text Rendering Style</label>
                      <select
                        value={visuals.textRenderStyle || 'standard'}
                        onChange={(e) => setVisuals(prev => ({ ...prev, textRenderStyle: e.target.value as any }))}
                        className="w-full bg-[#050508] border border-zinc-800 rounded p-2 text-xs text-white outline-none cursor-pointer transition-all"
                      >
                        <option value="standard">Standard Clean</option>
                        <option value="neon">Neon Glow (Audio Reactive)</option>
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-4">
                    ...
                    <div className="space-y-1.5 text-left">
                      <div className="flex justify-between items-center text-[10px] font-mono">
                        ...
                        <span className="text-brand-green">{customTextX}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                    ...
                    <div className="space-y-1.5 text-left pt-1">
                      <div className="flex justify-between items-center text-[10px] font-mono">
                        ...
                        <span className="text-brand-green">{customTextY}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-4">
                    ...
                    <div className="space-y-1.5 text-left">
                      <div className="flex justify-between items-center text-[10px] font-mono">
                        ...
                        <span className="text-brand-green">{visuals.textOpacity !== undefined ? visuals.textOpacity : 100}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                      <div className="flex items-center justify-between p-2 bg-[#050508] rounded border border-zinc-800/60">
                        ...
                        <button
                          ...
                          className={`relative w-7 h-4 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.textBeatScale ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                      ...
                      <div className="flex items-center justify-between p-2 bg-[#050508] rounded border border-zinc-800/60">
                        ...
                        <button
                          ...
                          className={`relative w-7 h-4 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.textBeatShake ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3 w-3 transition-all ${
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-3.5">
                        ...
                        return (
                          <button
                            ...
                            className={`w-10 h-10 flex items-center justify-center rounded-lg text-lg transition-all border cursor-pointer ${
                              isSelected
                                ? 'bg-zinc-900 border-brand-green/80 scale-105 shadow-md shadow-brand-green/10'
                    ...
                    {activeStickers.length > 0 && (
                      <div className="space-y-3.5 pt-3.5 border-t border-zinc-850/40 pl-2.5 border-l-2 border-brand-green/40 font-sans">
                        <h5 className="text-[9.5px] font-bold text-brand-green-hover uppercase font-mono text-left">Sticker Sizing & Coordinate Positioning</h5>
                        ...
                        <div className="space-y-1.5 text-left">
                          <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                            ...
                            <span className="text-white font-semibold flex items-center">
                              {visuals.stickerSize !== undefined ? visuals.stickerSize : Math.round(customTextFontSize * 0.9)} px
                            </span>
                          </div>
                          <input
                            type="range"
                            ...
                            max="120"
                            step="1"
                            value={visuals.stickerSize !== undefined ? visuals.stickerSize : Math.round(customTextFontSize * 0.9)}
                            onChange={(e) => setVisuals(prev => ({ ...prev, stickerSize: parseInt(e.target.value) }))}
                            className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                          />
                        </div>

                        {/* Sticker X Position slider */}
                        <div className="space-y-1.5 text-left font-sans">
                          <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                            <span>Sticker Horizontal Position (X)</span>
                            <span className="text-white font-semibold">
                              {visuals.stickerX !== undefined ? visuals.stickerX : customTextX} %
                            </span>
                          </div>
                          <input
                            type="range"
                            ...
                            max="100"
                            step="1"
                            value={visuals.stickerX !== undefined ? visuals.stickerX : customTextX}
                            onChange={(e) => setVisuals(prev => ({ ...prev, stickerX: parseInt(e.target.value) }))}
                            className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                          />
                        </div>

                        {/* Sticker Y Position slider */}
                        <div className="space-y-1.5 text-left font-sans">
                          <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                            <span>Sticker Vertical Position (Y)</span>
                            <span className="text-white font-semibold">
                              {visuals.stickerY !== undefined ? visuals.stickerY : Math.round(customTextY + 12)} %
                            </span>
                          </div>
                          <input
                            type="range"
                            ...
                            max="100"
                            step="1"
                            value={visuals.stickerY !== undefined ? visuals.stickerY : Math.round(customTextY + 12)}
                            onChange={(e) => setVisuals(prev => ({ ...prev, stickerY: parseInt(e.target.value) }))}
                            className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                          />
                        </div>
                      </div>
                  ...
                  {/* Advanced Text Motion & Effects */}
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-4">
                    <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide font-mono text-left border-b border-zinc-805/40 pb-1.5 flex items-center space-x-1.5">
                      <Wand2 className="w-3.5 h-3.5 text-brand-green-hover" />
                      <span>Advanced Text Effects</span>
                    </h4>

                    {/* Inverted mix-blend-mode */}
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-semibold text-zinc-350 block font-sans text-[11px] text-left">Inverted Mix-Blend Mode</span>
                        <span className="text-[10px] text-zinc-500 block font-sans mt-0.5 text-left">Apply canvas color inversion to typography and active stickers based on what is drawn behind them</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => setVisuals(prev => ({ ...prev, customTextInvertBlend: !prev.customTextInvertBlend }))}
                        className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                          visuals.customTextInvertBlend ? 'bg-brand-green' : 'bg-zinc-800'
                        }`}
                      >
                        <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                          visuals.customTextInvertBlend ? 'translate-x-3.5' : 'translate-x-0'
                        }`} />
                      </button>
                    </div>

                    {/* Marquee horizontal scrolling */}
                    <div className="space-y-3.5 pt-2 border-t border-zinc-850/40">
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="font-semibold text-zinc-350 block font-sans text-[11px] text-left">Lyric Marquee Scrolling</span>
                          <span className="text-[10px] text-zinc-500 block font-sans mt-0.5 text-left">Scroll text horizontally in a continuous marquee loop</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => setVisuals(prev => ({ ...prev, customTextMarquee: !prev.customTextMarquee }))}
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.customTextMarquee ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                            visuals.customTextMarquee ? 'translate-x-3.5' : 'translate-x-0'
                          }`} />
                        </button>
                      </div>

                      {visuals.customTextMarquee && (
                        <div className="space-y-1.5 text-left pl-2 border-l-2 border-brand-green/30">
                          <div className="flex justify-between items-center text-[10px] font-mono">
                            <span className="text-zinc-400 font-medium">Scroll Speed</span>
                            <span className="text-brand-green font-semibold">{visuals.customTextScrollSpeed || 2}</span>
                          </div>
                          <input
                            type="range"
                            min="1"
                            max="10"
                            step="1"
                            value={visuals.customTextScrollSpeed !== undefined ? visuals.customTextScrollSpeed : 2}
                            onChange={(e) => setVisuals(prev => ({ ...prev, customTextScrollSpeed: parseInt(e.target.value) }))}
                            className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                          />
                        </div>
                      )}
                  ...
                  <div className="bg-zinc-900/40 border border-zinc-900 rounded-lg p-4 space-y-4">
                    <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide font-mono text-left border-b border-zinc-850/40 pb-1.5 flex items-center space-x-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-brand-green-hover" />
                      <span>Watermark & Branding Systems</span>
                    </h4>

                    {/* Block A: TEXT BRANDING WATERMARK */}
                    <div className="p-3 bg-zinc-950/40 rounded border border-zinc-850/30 space-y-3.5">
                      <div className="flex items-center justify-between">
                        <div className="text-left">
                        ...
                        <button
                          ...
                          className={`relative w-8 h-4.5 rounded-full transition-all cursor-pointer flex-shrink-0 ${
                            visuals.renderWatermark ? 'bg-brand-green' : 'bg-zinc-800'
                          }`}
                        >
                          <span className={`absolute top-[2px] left-[2px] bg-zinc-100 rounded-full h-3.5 w-3.5 transition-all ${
                      ...
                      {visuals.renderWatermark && (
                        <div className="space-y-3.5 pl-2.5 border-l-2 border-brand-green/30 pt-1">
                          {/* Opacity slider */}
                          <div className="space-y-1.5 text-left">
                            <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                              <span>Watermark Opacity</span>
                              <span className="text-brand-green-hover font-semibold">
                                {Math.round((visuals.textWatermarkOpacity !== undefined ? visuals.textWatermarkOpacity : 0.35) * 100)} %
                              </span>
                            </div>
                            <input
                              type="range"
                              ...
                              max="1.0"
                              step="0.05"
                              value={visuals.textWatermarkOpacity !== undefined ? visuals.textWatermarkOpacity : 0.35}
                              onChange={(e) => setVisuals(prev => ({ ...prev, textWatermarkOpacity: parseFloat(e.target.value) }))}
                              className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                            />
                          </div>
                          ...
                          <div className="space-y-1.5 text-left">
                            <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                              <span>Font Size</span>
                              <span className="text-brand-green-hover font-semibold">
                                {visuals.textWatermarkFontSize !== undefined ? visuals.textWatermarkFontSize : (visuals.watermarkFontSize !== undefined ? visuals.watermarkFontSize : 12)} px
                              </span>
                            </div>
                            <input
                              type="range"
                              ...
                              max="48"
                              step="1"
                              value={visuals.textWatermarkFontSize !== undefined ? visuals.textWatermarkFontSize : (visuals.watermarkFontSize !== undefined ? visuals.watermarkFontSize : 12)}
                              onChange={(e) => setVisuals(prev => ({ ...prev, textWatermarkFontSize: parseInt(e.target.value) }))}
                              className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                            />
                          </div>

                          {/* Quick Alignment Tiles */}
                          <div className="space-y-1.5 text-left">
                            <label className="block text-[9px] font-mono text-zinc-400 font-semibold uppercase">Quick Alignment Corners</label>
                            <div className="grid grid-cols-4 gap-1.5">
                              {[
                                { label: 'Top Left', val: 'top-left' },
                                { label: 'Bottom Cent', val: 'bottom-center' },
                                { label: 'Bottom Right', val: 'bottom-right' },
                                { label: 'Manual', val: 'manual' }
                              ].map((tile) => {
                                const isActive = (visuals.textWatermarkAlignment || 'bottom-right') === tile.val;
                                return (
                                  <button
                                    key={`text-align-${tile.val}`}
                                    type="button"
                                    onClick={() => setVisuals(prev => ({ ...prev, textWatermarkAlignment: tile.val as any }))}
                                    className={`px-1 py-1.5 text-[9px] font-sans font-medium text-center rounded border transition-all cursor-pointer truncate ${
                                      isActive
                                        ? 'bg-brand-green/15 border-brand-green text-white font-bold'
                                        : 'bg-zinc-950 border-zinc-850 text-zinc-500 hover:bg-zinc-900 hover:text-zinc-400'
                                    }`}
                                  >
                                    {tile.label}
                                  </button>
                          ...
                          {/* Manual positioning */}
                          {visuals.textWatermarkAlignment === 'manual' && (
                            <div className="space-y-2.5 pt-2 border-t border-zinc-900/50">
                              <div className="space-y-1 text-left">
                                <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                  <span>Horizontal Position (X)</span>
                                  <span className="text-white font-semibold">{visuals.textWatermarkX !== undefined ? visuals.textWatermarkX : 90} %</span>
                                </div>
                                <input
                                  type="range"
                                  min="0"
                                  max="100"
                                  step="1"
                                  value={visuals.textWatermarkX !== undefined ? visuals.textWatermarkX : 90}
                                  onChange={(e) => setVisuals(prev => ({ ...prev, textWatermarkX: parseInt(e.target.value) }))}
                                  className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                                />
                              </div>
                              <div className="space-y-1 text-left">
                                <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                  ...
                                  <span className="text-white font-semibold">{visuals.textWatermarkY !== undefined ? visuals.textWatermarkY : 90} %</span>
                                </div>
                                <input
                                  type="range"
                                  min="0"
                                  max="100"
                                  step="1"
                                  value={visuals.textWatermarkY !== undefined ? visuals.textWatermarkY : 90}
                                  onChange={(e) => setVisuals(prev => ({ ...prev, textWatermarkY: parseInt(e.target.value) }))}
                                  className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                                />
                              </div>
                            </div>
                    ...
                    {/* Block B: LOGO IMAGE WATERMARK */}
                    <div className="p-3 bg-zinc-950/40 rounded border border-zinc-850/30 space-y-3.5">
                      <div className="text-left">
                        <span className="font-semibold text-zinc-350 block font-sans text-[11px] uppercase tracking-wide font-bold">Logo Image Watermark</span>
                        <span className="text-[9.5px] text-zinc-500 block font-sans mt-0.5">Upload a transparent vector or branding PNG asset to overlay on the canvas</span>
                      </div>
                      ...
                      {visuals.watermarkUrl ? (
                        <div className="space-y-3.5">
                          <div className="flex items-center justify-between p-2 bg-zinc-950 rounded border border-zinc-850">
                            <div className="flex items-center space-x-2.5 overflow-hidden mr-2">
                              <img src={visuals.watermarkUrl} className="w-8 h-8 rounded object-contain bg-zinc-950 border border-zinc-850" alt="Watermark preview" referrerPolicy="no-referrer" />
                              <span className="text-[10px] text-zinc-400 truncate font-mono">Custom Logo Active</span>
                            </div>
                            <button
                              onClick={handleClearWatermark}
                              type="button"
                              className="text-[10px] text-red-400 hover:text-red-350 transition-colors font-semibold px-2 py-1 rounded bg-red-950/30 border border-red-900/50 hover:bg-red-900/40 cursor-pointer"
                            >
                              Clear
                            </button>
                          </div>

                          <div className="space-y-3 pl-2.5 border-l-2 border-brand-green/30 pt-1">
                            {/* Opacity slider */}
                            <div className="space-y-1.5 text-left">
                              <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                <span>Logo Opacity</span>
                                <span className="text-brand-green-hover font-semibold">
                                  {Math.round((visuals.logoWatermarkOpacity !== undefined ? visuals.logoWatermarkOpacity : (visuals.watermarkOpacity !== undefined ? visuals.watermarkOpacity : 0.8)) * 100)} %
                                </span>
                              </div>
                              <input
                                type="range"
                                ...
                                max="1.0"
                                step="0.05"
                                value={visuals.logoWatermarkOpacity !== undefined ? visuals.logoWatermarkOpacity : (visuals.watermarkOpacity !== undefined ? visuals.watermarkOpacity : 0.8)}
                                onChange={(e) => setVisuals(prev => ({ ...prev, logoWatermarkOpacity: parseFloat(e.target.value) }))}
                                className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                              />
                            </div>
                            ...
                            <div className="space-y-1.5 text-left">
                              <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                ...
                                <span className="text-brand-green-hover font-semibold">
                                  {Math.round(visuals.logoWatermarkScale !== undefined ? visuals.logoWatermarkScale : (visuals.watermarkScale !== undefined ? visuals.watermarkScale : 100))}%
                                </span>
                              </div>
                              <input
                                type="range"
                                ...
                                max="200"
                                step="5"
                                value={visuals.logoWatermarkScale !== undefined ? visuals.logoWatermarkScale : (visuals.watermarkScale !== undefined ? visuals.watermarkScale : 100)}
                                onChange={(e) => setVisuals(prev => ({ ...prev, logoWatermarkScale: parseInt(e.target.value) }))}
                                className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                              />
                            </div>
                            ...
                            <div className="space-y-1.5 text-left">
                              ...
                              <div className="grid grid-cols-4 gap-1.5">
                                {[
                                  ...
                                  const isActive = (visuals.logoWatermarkAlignment || 'top-right') === tile.val || (tile.val === 'bottom-center' && visuals.logoWatermarkAlignment === 'bottom-center');
                                  return (
                                    <button
                                      key={`logo-align-${tile.val}`}
                                      type="button"
                                      onClick={() => setVisuals(prev => ({ ...prev, logoWatermarkAlignment: tile.val as any }))}
                                      className={`px-1 py-1.5 text-[9px] font-sans font-medium text-center rounded border transition-all cursor-pointer truncate ${
                                        isActive
                                          ? 'bg-lime-600/15 border-brand-green text-white font-bold'
                                          : 'bg-zinc-950 border-zinc-850 text-zinc-500 hover:bg-zinc-900 hover:text-zinc-400'
                                      }`}
                                    >
                                      {tile.label}
                            ...
                            {/* Manual positioning */}
                            {visuals.logoWatermarkAlignment === 'manual' && (
                              <div className="space-y-2.5 pt-2 border-t border-zinc-900/50">
                                <div className="space-y-1 text-left">
                                  <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                    <span>Horizontal Position (X)</span>
                                    <span className="text-white font-semibold">{visuals.logoWatermarkX !== undefined ? visuals.logoWatermarkX : (visuals.watermarkX !== undefined ? visuals.watermarkX : 90)} %</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="0"
                                    max="100"
                                    step="1"
                                    value={visuals.logoWatermarkX !== undefined ? visuals.logoWatermarkX : (visuals.watermarkX !== undefined ? visuals.watermarkX : 90)}
                                    onChange={(e) => setVisuals(prev => ({ ...prev, logoWatermarkX: parseInt(e.target.value) }))}
                                    className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                                  />
                                </div>
                                <div className="space-y-1 text-left">
                                  <div className="flex justify-between font-mono text-[9px] text-zinc-400">
                                    <span>Vertical Position (Y)</span>
                                    <span className="text-white font-semibold">{visuals.logoWatermarkY !== undefined ? visuals.logoWatermarkY : (visuals.watermarkY !== undefined ? visuals.watermarkY : 10)} %</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="0"
                                    max="100"
                                    step="1"
                                    value={visuals.logoWatermarkY !== undefined ? visuals.logoWatermarkY : (visuals.watermarkY !== undefined ? visuals.watermarkY : 10)}
                                    onChange={(e) => setVisuals(prev => ({ ...prev, logoWatermarkY: parseInt(e.target.value) }))}
                                    className="w-full h-1 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-brand-green"
                                  />
                                </div>
                              </div>
                        ...
                        <div className="relative border border-dashed border-zinc-805 hover:border-zinc-750 bg-zinc-950/30 hover:bg-zinc-950/50 rounded p-4 text-center group transition-all">
                          <input
                            type="file"
                            accept="image/png"
                          ...
                          <div className="flex flex-col items-center justify-center space-y-1 pointer-events-none">
                            <span className="text-zinc-350 font-sans text-[11px] group-hover:text-zinc-200 transition-colors font-medium">Upload custom PNG logo</span>
                            <span className="text-zinc-605 text-[9px] font-mono">Transparent background recommended</span>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between border-b border-zinc-800/80 pb-3">
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          className="w-3.5 h-3.5 rounded border border-zinc-850 bg-zinc-950 text-brand-green focus:ring-brand-green cursor-pointer accent-brand-green"
                        />
                    ...
                    {subtitles.enabled && (
                      <div className="space-y-4">
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            <label className="text-[10px] font-mono font-bold text-zinc-400 uppercase tracking-wide flex items-center space-x-2">
                              ...
                              {subtitles.lyrics.length > 0 && (
                                <span className="text-brand-green-hover">Parsed: {subtitles.lyrics.length} lines</span>
                              )}
                            </label>
                            <div>
                              <input
                          ...
                          <textarea
                            className="w-full bg-zinc-950 border border-zinc-805 rounded p-2 text-[10px] font-mono text-zinc-300 h-28 focus:outline-none focus:border-brand-green resize-y placeholder:text-zinc-700"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1.5">
                            ...
                            <select
                              className="w-full bg-zinc-950 border border-zinc-805 rounded p-1.5 text-[10px] font-mono text-zinc-300 focus:outline-none focus:border-brand-green cursor-pointer"
              ...
              >
                ...
                <div className="bg-zinc-950/40 p-4 rounded-xl border border-zinc-900/50 space-y-3 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center relative z-10">
                    ...
                    <span className="text-[11px] font-bold text-brand-green-hover font-mono">
                  ...
                  <div className="flex justify-between items-center relative z-10">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-[9.5px] font-mono rounded border transition-all cursor-pointer ${
                        sfxEcho
                          ? 'bg-lime-650/20 border-brand-green/50 text-brand-green-hover font-bold'
                  ...
                  <div className="flex justify-between items-center relative z-10">
                    ...
                    <div className="flex items-center space-x-1.5">
                      <input
                        ...
                        className="w-3.5 h-3.5 rounded border border-zinc-850 bg-zinc-950 text-brand-green focus:ring-brand-green accent-brand-green cursor-pointer"
                      />
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  {!isExporting ? (
                    ...
                    <div className="bg-brand-green/5 border border-brand-green/10 rounded-lg p-3 text-center text-xs text-brand-green-hover font-mono animate-pulse">
                  ...
                  {exportedVideoUrl && (
                    ...
                    >
                      <div className="flex items-center space-x-2.5">
                        <div className="w-7 h-7 rounded-sm bg-brand-green/15 flex items-center justify-center border border-brand-green/20">
                          <Check className="w-4 h-4 text-brand-green-hover" />
                        </div>
                        <div>
                      ...
                      <a
                        ...
                        className="w-full py-2.5 px-3 bg-brand-green hover:bg-brand-green-hover text-white rounded font-mono text-xs font-semibold text-center transition-all flex items-center justify-center space-x-1.5 shadow-md"
                      >
                        <Download className="w-4 h-4" />
      ...
      <AnimatePresence>
        {pwaUpdateAvailable && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-[200] max-w-sm w-full bg-lime-950/90 backdrop-blur-md border border-brand-green/30 p-4 rounded-xl shadow-2xl overflow-hidden"
          >
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-lime-600 via-brand-green to-brand-green-hover"></div>
            <div className="flex flex-col space-y-3">
              <div className="flex items-start space-x-3">
                <div className="w-8 h-8 rounded-full bg-lime-900/50 flex items-center justify-center shrink-0 border border-brand-green/20">
                  <span className="text-brand-green-hover text-lg">⚠️</span>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-lime-100 uppercase tracking-wide font-sans">System Update Available</h4>
                  <p className="text-[11px] text-lime-300/80 leading-snug mt-1 font-sans">
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t border-brand-green/20">
                <button
                  onClick={() => setPwaUpdateAvailable(false)}
                  className="px-3 py-1.5 text-[10px] font-semibold text-lime-300 hover:text-white transition-colors"
                >
                  Dismiss
                </button>
                <button
                  onClick={() => window.location.reload()}
                  className="px-3 py-1.5 bg-brand-green hover:bg-brand-green-hover text-white rounded text-[10px] font-bold shadow-lg transition-colors"
                >
                  Refresh Now
                </button>
              </div>
